...
echo "$($PSQL "SELECT DISTINCT t.name FROM games g INNER JOIN teams t ON g.winner_id = t.team_id OR g.opponent_id = t.team_id WHERE g.year = 2014 AND g.round = 'Eighth-Final' ORDER BY t.name")"
